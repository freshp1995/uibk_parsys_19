...
int main(int argc, char **argv) {
    if (argc < 4) {
		printf("First parameter is the number of particles for each rank, second parameter is the number of timestamps, third is i seriell or not\n");
		return EXIT_FAILURE;
	}
    ...
    Particle *root;
    if (seriell == 1) {
        root = create_particles(numberParticles);
    }
